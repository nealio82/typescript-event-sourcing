...
describe('Aggregate', () => {
    ...
    test('exception is thrown when no apply... method exists', () => {
        ...
        const execution = () => {
            aggregate.raise(
                new (class UnhandledEvent implements AggregateEvent {
                    aggregateId(): AggregateId {
                        return new (class extends AggregateId {
                            toString(): string {
                                return '12345'
        ...
        expect(execution).toThrow(
            'The "SampleAggregate" class does not yet have a function named "applyUnhandledEvent" implemented'
        )
    })
    ...
    test('aggregate cannot be built from existing events if other events have already been applied', () => {
        ...
        const execution = () => {
            Aggregate.buildFromEvents([firstEvent, secondEvent], aggregate)
        }

        expect(execution).toThrow(Error)
        expect(execution).toThrow(
            'Cannot rebuild aggregate from events if other events have already been applied'
        )
    })
    ...
    test('events are flushed', () => {
        ...
        expect(aggregate.flush()).toEqual([firstEvent, secondEvent, thirdEvent])
    })
    ...
    test('new events are added after aggregate has been rebuilt from previous', () => {
        const aggregate = Aggregate.buildFromEvents(
            [new FirstEvent(), new SecondEvent()],
            new SampleAggregate()
        )
