...
abstract class Aggregate {
    ...
    public static buildFromEvents<T extends Aggregate>(
        events: Array<AggregateEvent>,
        aggregate: T
    ): T {
    ...
    private apply(event: AggregateEvent): void {
        ...
        if (typeof (this as any)[method] !== 'function') {
            throw new ReferenceError(
                `The "${aggregateName}" class does not yet have a function named "${method}" implemented`
            )
        }
